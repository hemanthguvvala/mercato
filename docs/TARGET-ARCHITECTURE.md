...
>
> **Progress (2026-07-20):** Slices 0–6 ✅ (Slice 4 = inbox + F21 dedup done; itemized-events / participant-outbox tail deferred) · **Slice 7 (observability) in progress.** See §6.
...
- **Slice 4 — Real events + outbox/inbox.** ✅ **PARTIAL (2026-07-20).** Inbox dedup on inventory's `OrderPlaced` consumer + claim-before-effect (F21) on notification/analytics done. **Deferred tail:** itemized/renamed events (`OrderConfirmed`, `StockReserved`, `PaymentAuthorized`) + participant (inventory/payment) outboxes. *Concept: event-carried state transfer, inbox/outbox, at-least-once made safe.*
- **Slice 5 — CQRS read model.** ✅ **DONE (2026-07-20).** `OrderView` + `OrderViewProjector` (idempotent upsert from `OrderPlaced`, own `order-view` consumer group); `GET /orders` reads it (user-scoped + paginated), `GET /orders/{id}` authz-scoped. Verified live (projection lag, pagination, authz — a foreign id → 404). *Concept: CQRS read/write split.*
- **Slice 6 — Resilience on the money path.** ✅ **DONE (2026-07-20).** Per-participant circuit breakers + fallbacks: `InventoryGateway` (reserve) and `PaymentWebClient` (charge, + 3s response timeout). Business outcomes converted/ignored so they never trip the breaker (out-of-stock 404; 402 decline → `PaymentDeclinedException`); refund left un-breakered (never fast-fail compensation); saga compensates on both failure modes. **Still TODO:** gateway timeout+CB, rate-limit key fix. *Concept: bulkhead the critical path; stop the cascade.*
- **Slice 7 — Observability end-to-end.** 🚧 **IN PROGRESS (2026-07-20).** Kafka trace propagation on (template+listener observation) so publish→consume is one trace; business metrics on the order flow (`orders.placed{outcome}` counter, `orders.processing` timer) via Micrometer→Prometheus; trace-id log correlation auto (Boot + micrometer-tracing). **TODO:** store `traceparent` in the outbox row to bridge the HTTP→publish gap; dashboards; PII scrub. *Concept: trace one order across every hop.*
