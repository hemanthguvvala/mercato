...
- **Build:**
  ...
  - [ ] **Resilience4j** on the Feign call: retry → circuit breaker → bulkhead → rate limiter.
...
- **DoD:** Catalog reads hit Redis; killing Catalog trips the breaker (Order degrades, doesn't hang); cache is consistent after an update.
...
- **Build:**
  ...
  - [ ] **Notification service** consumes → mock email (log).
  - [ ] Second consumer group (Analytics) → demonstrate fan-out.
- **Why:** decouples side-effects from the request path; multiple consumers react independently.
- **DoD:** placing an order produces an event; Notification + Analytics both react without Order knowing about them. **🏁 This is already a credible portfolio piece.**
