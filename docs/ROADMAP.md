...
>
> **Update (2026-07-19):** since the 2026-07-18 production audit, a parallel **production-hardening
> track** runs alongside these phases — see [`PRODUCTION-FIX-PLAN.md`](./PRODUCTION-FIX-PLAN.md),
> [`TARGET-ARCHITECTURE.md`](./TARGET-ARCHITECTURE.md), [`ENHANCEMENT-BACKLOG.md`](./ENHANCEMENT-BACKLOG.md).
> The substrate has moved on from what several phases below assume: **all 5 stateful services now run
> on Neon Postgres** (the Phase-0 "Neon swap later" is done), the stack is on **Spring Boot 3.5.15 /
> Cloud 2025.0.3**, and order-service has an explicit **state machine** + `OrderStatusHistory` (Slice 1).
...
- **Build:**
  ...
  - [x] DB: use **H2 file-mode** (persistent, free, zero-install) now; optional one-line swap to free **Neon** Postgres later for the portfolio. (No local Docker — see Decisions Log.) ✅ 2026-06-21 → **Neon swap done 2026-07-19** — all 5 stateful services on Neon Postgres (database-per-service), Flyway retained (Slice 0 / F3).
...
- **Build:**
  ...
  - [x] **Transactional Outbox** so DB-commit and event-publish can't diverge. `create()` writes the order **+** an `outbox` row in ONE `@Transactional` DB tx (no dual-write); an `OutboxPoller` (`@Scheduled(fixedDelay=2000)` + `@Transactional`) reads `findByPublishedFalse()`, sends to Kafka, **waits for the ack (`.get()`)** *before* `markPublished()` (dirty-checked UPDATE) → at-least-once. Verified live incl. the **Kafka-down test**: order still 201, row parked `published=false`, drains to `true` on Kafka restart, zero loss. ✅ 2026-06-23
  - [x] **Idempotency keys** (Redis) on consumers. Both notification + analytics dedupe via `StringRedisTemplate.setIfAbsent("<consumer>:OrderPlaced:<orderId>", "1", 24h TTL)` — **per-consumer key prefix** so each processes the event once independently; `true`→first time→process, `false`→duplicate→skip (`Boolean.FALSE.equals`, null-safe). Handles the at-least-once dups the outbox can produce. Verified via kafka-console-producer (duplicate orderId skipped by both, new id processed). ✅ 2026-06-24
  - [~] **Concurrency / threading module:** reproduced the oversell race with a 20-thread stampede (`InventoryConcurrencyTest`, `ExecutorService` + `CountDownLatch` start/done gates) and compared **optimistic** (`@Version`) vs **pessimistic** (`@Lock(PESSIMISTIC_WRITE)` on `findByProductIdForUpdate` (FOR UPDATE) + `reservePessimistic`). Both proven to **never oversell**; optimistic fails-and-retries (can *under*-sell under contention → losers don't retry), pessimistic serializes (0 version failures, sells all). ✅ 2026-06-24 (commit 36486ef). ⏸ **Redisson distributed lock + virtual threads deferred (optional):** for a single shared DB the DB lock already prevents oversell across all pods — a distributed lock only earns its keep when there's no single DB row (cross-system / run-on-one-instance). Revisit with the Java 21 bump.
...
- **Build:**
  - [~] **gRPC** Order→Inventory: wrote `inventory.proto`, codegen via **ascopes protobuf-maven-plugin** (xolstice 0.6.1 was broken) + build-helper for IDE source folders, and a `@GrpcService` `InventoryGrpcService` (net.devh starter) on :9090 delegating to InventoryService — **compiles + runs as a demo**. Client/saga-wiring **deferred** (gRPC tooling friction). Evaluated official Spring gRPC but it needs **Spring Boot 4.0** (major bump), so stayed on net.devh. 2026-06-25
  - [x] **GraphQL** on Catalog: `schema.graphqls` (Product type + `product(id)`/`products(name)` queries) + a `@Controller` with `@QueryMapping`/`@Argument` resolvers delegating to ProductService. **Verified via `@GraphQlTest` (2 green)** — schema loads, args coerce (ID→Long), client gets only the requested fields. ✅ 2026-06-25
  - [x] **WebClient** Order→Payment: `@LoadBalanced WebClient.Builder` + `PaymentWebClient` (`.post().uri().bodyValue().retrieve().block()`); saga swapped off Feign, catch broadened to `FeignException | WebClientResponseException` so compensation still fires on a 402. Compiles. (Honest: `.block()` in the sync saga forfeits the non-blocking benefit — done for learning; RestClient would've fit this call better.) ✅ 2026-06-25
...
> **Parallel production-hardening track (2026-07-18 →), tracked separately** in
> [`PRODUCTION-FIX-PLAN.md`](./PRODUCTION-FIX-PLAN.md) (F1–F39) + [`TARGET-ARCHITECTURE.md`](./TARGET-ARCHITECTURE.md) (Slices 0–7):
> **Slice 0 ✅** substrate → Neon Postgres (5 services) + Boot 3.5.15 / Cloud 2025.0.3 + security quick wins ·
> **Slice 1 ✅** order state machine + `OrderStatusHistory` · **Slice 2 = next** (durable saga reconciler + inventory reservation expiry).
...
- _2026-06-23 — Reliable event publish: outbox vs direct send_ — **Transactional Outbox.** Direct `kafkaTemplate.send()` inside `@Transactional create()` is the **dual-write problem**: DB + Kafka are two systems with no shared transaction. Async `send()` (no `.get()`) silently **loses** the event if Kafka is down (order commits anyway); forcing it sync with `.get()` either **fails the order whenever Kafka is down** (availability coupling) or still **races** (Kafka accepted, DB then rolls back → phantom event). You can't atomically commit two systems without 2PC. **Fix = collapse to ONE system:** `create()` writes order + an `outbox` row in one DB tx (truly atomic); a separate `@Scheduled` poller ships rows to Kafka later with retries. Impl rules learned: poller must **wait for the broker ack (`.get()`) BEFORE `markPublished()`** (else it marks "sent" when an async send silently failed); send-then-mark ordering = **at-least-once** (dups possible → idempotency is the next item); the poller and `create()` are **separate transactions on separate threads** — a poller rollback can't undo the already-committed order; and `@Transactional` governs the **database only, never Kafka**.
- _2026-07-19 — Substrate hardening (post-audit): migrate to Postgres now, db-per-service, and bump Boot?_ — The 2026-07-18 audit flagged **H2-file as the single-replica scale blocker**, so migrated all 5 stateful services to **Neon Postgres** with **database-per-service on one instance** (each service its own logical DB → strong isolation, no cross-service FK — the most common real-world split; separate *instances* are a later scale/blast-radius move, not needed yet). Hit `Unsupported Database: PostgreSQL` — root cause was **not** the PG version but the missing **`flyway-database-postgresql`** module (Flyway 10+ split DB support out of flyway-core). Fixing that surfaced a Boot-autoconfig incompatibility, so also bumped **Spring Boot 3.3.5 → 3.5.15 / Spring Cloud 2023.0.3 → 2025.0.3** (14/14 modules build). **Lesson:** a Flyway "unsupported database" almost always means a missing DB module, not a version mismatch.
